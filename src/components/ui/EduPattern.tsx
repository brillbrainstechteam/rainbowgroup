...
type PatternProps = {
  ...
  scale?: number;
  id?: string;
  /** Give each glyph its own spectrum hue instead of inheriting currentColor. */
  spectrum?: boolean;
};

/** Spectrum hues in rainbow order, one per glyph in the tile. */
const SPECTRUM = ["#6A5AC8", "#3E6FCB", "#2E9C8E", "#D9A441", "#C4604F"];
...
export function EduPattern({
  ...
  const hue = (i: number) => (spectrum ? SPECTRUM[i] : "currentColor");
    ...
    >
      <defs>
        ...
        >
          ...
          >
            {/* Graduation cap */}
            <g transform="translate(18 20) scale(0.85)" stroke={hue(0)}>
            ...
            {/* Open book */}
            <g transform="translate(150 44) scale(0.72) rotate(9)" stroke={hue(1)}>
            ...
            {/* Pencil */}
            <g transform="translate(38 150) scale(0.78) rotate(-14)" stroke={hue(2)}>
...
type AccentProps = {
  ...
  className?: string;
  opacity?: number;
  /** Explicit stroke colour; falls back to inherited currentColor. */
  color?: string;
};
